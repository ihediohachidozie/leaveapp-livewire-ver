<div class="p-6">
                            ...
                            @if ($data->count())
                                @foreach ($data as $item)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 text-sm whitespace-no-wrap">{{ $item->days}}</td>
    ...
    {{ $data->links()}}
        ...
        <x-slot name="content">
            <div class="mt-4">
                <x-jet-label for="name" value="{{ __('Staff Category') }}" />
                <x-jet-input wire:model.lazy="name" id="name" class="block mt-1 w-full" type="text" />
                @error('name') <span class="error">{{$message}}</span> @enderror
            </div>
            <div class="mt-4">
                <x-jet-label for="days" value="{{ __('Approved Days') }}" />
                <x-jet-input wire:model.lazy="days" id="days" class="block mt-1 w-full" type="number"/>
    ...
    <x-jet-dialog-modal wire:model="modalConfirmDeleteVisible">
        ...
        <x-slot name="content">
            {{ __('Are you sure you want to delete the category? Once the category is deleted, all of its resources and data will be permanently deleted.') }}
